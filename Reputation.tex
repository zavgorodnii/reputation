...
Formula for calculating the rating based on the Bernoulli distribution model, a discrete probability distribution, that mimics a random experiment, with a known fact of success or failure. The Bernoulli distribution is a special case of the Binomial distribution where a single experiment/trial is conducted (n=1). It is also a special case of the two-point distribution, for which the outcome need not be a bit, i.e., the two possible outcomes need not be 0 and 1.
